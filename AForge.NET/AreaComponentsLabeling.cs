...
namespace AForge.Imaging.Filters
{
	...
	public class AreaComponentsLabeling : BaseFilter
	{
		...
		{Color.DarkBlue, Color.LimeGreen, Color.Yellow, Color.Red, Color.SlateGray };
		...
		public int Alv { get; set; } = 1000;
		public int Des { get; set; } = 10000;
				...
				for (int x = 0; x<width; x++, dst+=3, p++) {
					if (labels[p]!=0) {
						if (fillshape||(p-width-1<0)||(p+width+1>(width*height-1))||
							  ...
							  labels[p+width-1]==0||labels[p+width]==0||labels[p+width+1]==0) {
							Color c = colorTable[4]; // default to ceiling
							if (blobs[labels[p]-1].Area<Low) { c=colorTable[0]; } // <floor
							else if (blobs[labels[p]-1].Area<Alv) { c=colorTable[1]; } // >=floor & <low
							else if (blobs[labels[p]-1].Area<=Des) { c=colorTable[2]; } // >=low & <=high
									else if (blobs[labels[p]-1].Area<=High) { c=colorTable[3]; } // >high & <=ceiling                        

...
namespace AForge.Imaging
{
	...
	/// </remarks>
	/// 
	public abstract class BlobxCounterBase
	{
		// found blobs
		public List<Blobx> blobs = new List<Blobx>();

		// objects' sort order
		private ObjectxsOrder objectsOrder = ObjectxsOrder.None;

		// filtering by size is required or not
		private bool filterBlobs = false;
		private IBlobxsFilter filter = null;
		...
		/// </para></remarks>
		/// 
		public ObjectxsOrder ObjectsOrder {
		...
		/// </remarks>
		/// 
		public IBlobxsFilter BlobsFilter {
			get { return filter; }
			set { filter=value; }
		}
		...
		/// 
		public BlobxCounterBase(Bitmap image)
		{
			ProcessImage(image);
		}
		...
		/// 
		public BlobxCounterBase(BitmapData imageData)
		{
		...
		/// 
		public BlobxCounterBase(UnmanagedImage image)
		{
			ProcessImage(image);
		}
		...
		public void ProcessImage(UnmanagedImage image)
		{
			...
			if (objectsOrder!=ObjectxsOrder.None) {
				blobs.Sort(new BlobxsSorter(objectsOrder));
			}
		}
		...
		/// can not be collected.</exception>
		/// 
		public Blobx[] GetObjectsInformation()
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");

			Blobx[] blobsToReturn = new Blobx[objectsCount];

			// create each blob
			for (int k = 0; k<objectsCount; k++) {
				blobsToReturn[k]=new Blobx(blobs[k]);
			}
		...
		/// can not be collected.</exception>
		/// 
		public Blobx[] GetObjects(Bitmap image, bool extractInOriginalSize)
		{
			Blobx[] blobs = null;
			// lock source bitmap data
			BitmapData imageData = image.LockBits(
		...
		/// can not be collected.</exception>
		/// 
		public Blobx[] GetObjects(UnmanagedImage image, bool extractInOriginalSize)
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");
			...
			Blobx[] objects = new Blobx[objectsCount];
		...
		/// 
		public void ExtractBlobsImage(Bitmap image, Blobx blob, bool extractInOriginalSize)
		{
			// lock source bitmap data
			BitmapData imageData = image.LockBits(
				 new Rectangle(0, 0, image.Width, image.Height),
		...
		/// can not be extracted.</exception>
		/// 
		public void ExtractBlobsImage(UnmanagedImage image, Blobx blob, bool extractInOriginalSize)
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");
		...
		/// can not be extracted.</exception>
		/// 
		public void GetBlobsLeftAndRightEdges(Blobx blob, out List<IntPoint> leftEdge, out List<IntPoint> rightEdge)
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");
		...
		/// can not be extracted.</exception>
		/// 
		public void GetBlobsTopAndBottomEdges(Blobx blob, out List<IntPoint> topEdge, out List<IntPoint> bottomEdge)
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");
		...
		/// can not be extracted.</exception>
		/// 
		public List<IntPoint> GetBlobsEdgePoints(Blobx blob)
		{
			// check if objects map was collected
			if (objectLabels==null)
				throw new ApplicationException("Image should be processed before to collect objects map.");
		...
		private unsafe void CollectObjectsInfo(UnmanagedImage image)
		{
			...
			int[] perimeter = new int[objectsCount+1];
			...
			if (image.PixelFormat==PixelFormat.Format8bppIndexed) {
				...
				for (int y = 0; y<imageHeight; y++) {
					for (int x = 0; x<imageWidth; x++, i++, src++) {
						label=objectLabels[i]; // get current label

						if (label==0) continue; // skip unlabeled pixels
						...
						area[label]++; // sum up area

						if ((i-imageWidth-1)<0||(i+imageWidth+1)>(imageWidth*imageHeight-1)||
							  objectLabels[i-imageWidth-1]==0||objectLabels[i-imageWidth]==0||objectLabels[i-imageWidth+1]==0||
							  objectLabels[i-1]==0||objectLabels[i+1]==0||
							  objectLabels[i+imageWidth-1]==0||objectLabels[i+imageWidth]==0||objectLabels[i+imageWidth+1]==0)
							perimeter[label]++; // sum up perimeter (8-way)

						//if ( (i-imageWidth-1)<0 || (i+imageWidth+1)>(imageWidth*imageHeight-1) ||
						//     objectLabels[i-imageWidth]==0 || 
						//     objectLabels[i-1]==0 || objectLabels[i+1]==0 || 
						//     objectLabels[i+imageWidth]==0  )
						//    perimeter[label]++; // sum up perimeter (4-way)
			...
			for (int j = 1; j<=objectsCount; j++) {
				Blobx blob = new Blobx(j, new Rectangle(x1[j], y1[j], x2[j]-x1[j]+1, y2[j]-y1[j]+1));
				int blobArea = area[j];
				blob.Area=blobArea;
				blob.Perimeter=perimeter[j];
		...
		private class BlobxsSorter : System.Collections.Generic.IComparer<Blobx>
		{
			private ObjectxsOrder order;

			public BlobxsSorter(ObjectxsOrder order)
			{
				this.order=order;
			}

			public int Compare(Blobx a, Blobx b)
			{
				Rectangle aRect = a.Rectangle;
				Rectangle bRect = b.Rectangle;

				switch (order) {
					case ObjectxsOrder.Size: // sort by size

						// the order is changed to descending
						return (bRect.Width*bRect.Height-aRect.Width*aRect.Height);

					case ObjectxsOrder.Area: // sort by area
						return b.Area-a.Area;

					case ObjectxsOrder.YX:   // YX order

						return ((aRect.Y*100000+aRect.X)-(bRect.Y*100000+bRect.X));

					case ObjectxsOrder.XY:   // XY order

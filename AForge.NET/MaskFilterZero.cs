...
namespace AForge.Imaging.Filters
{
	...
	/// </remarks>
	/// 
	public class MaskFilterZero : BaseInPlacePartialFilter
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="MaskFilterZero"/> class.
		/// </summary>
		/// 
		...
		/// 
		public MaskFilterZero(IFilter baseFiler, Bitmap maskImage)
		{
		...
		/// 
		public MaskFilterZero(IFilter baseFiler, UnmanagedImage unmanagedMaskImage)
		{
			BaseFilter=baseFiler;
		...
		/// 
		public MaskFilterZero(IFilter baseFiler, byte[,] mask)
		{
			BaseFilter=baseFiler;
			Mask=mask;
		}
						...
						for (int y = startY; y<stopY; y++) {
							...
							for (int x = startX; x<stopX; x++, imagePtr+=3, filteredPtr+=3, mask++) {
								if (*mask==0) {
									...
									imagePtr[RGB.R]=65535;
									imagePtr[RGB.G]=65535;
									imagePtr[RGB.B]=65535;
								}
							}
							imagePtrBase+=srcStride;
							filteredPtrBase+=filteredStride;
